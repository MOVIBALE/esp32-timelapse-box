  ...
  for (const auto &i : index) {
    ...
    switch (i.type) {
      ...
      case Camera::Type::MOBILE_DEVICE:
        ESP_LOGW(LOG_TAG, "Ignoring unsupported saved camera type %lu",
...
bool CameraList::match(const NimBLEAdvertisedDevice *pDevice) {
  for (const auto &camera : m_ConnectList) {
    if (camera->getAddress() == pDevice->getAddress()) return false;
  }
...
void CameraList::addFauxNY(void) {
  ESP_LOGW(LOG_TAG, "FauxNY is not included in the multibrand experiment");
}
