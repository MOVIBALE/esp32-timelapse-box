...
void Ricoh::onPassKeyEntry(NimBLEConnInfo &connInfo) {
  uint32_t pin = 0;
  const auto address = connInfo.getAddress().toString();
  if (PairingApproval::requestPasskey(pin, address.c_str())) {
    NimBLEDevice::injectPassKey(connInfo, pin);
  }
}

uint32_t Ricoh::onPassKeyDisplay(NimBLEConnInfo &connInfo) {
  const uint32_t pin = esp_random() % 1000000;
  const auto address = connInfo.getAddress().toString();
  PairingApproval::showPasskey(pin, address.c_str());
  return pin;
}

void Ricoh::onConfirmPasskey(NimBLEConnInfo &connInfo, uint32_t pin) {
  const auto address = connInfo.getAddress().toString();
  const bool accepted = PairingApproval::requestConfirmation(pin, address.c_str());
