...
- 中文：ESP32 延时摄影盒子
- 规范 Klipper 宏：ESP32_TIMELAPSE_SHOT
...
本次打印启用？仅当 Klipper 已安装 ESP32_TIMELAPSE_SHOT 宏时才继续。
...
新切片必须只输出 ESP32_TIMELAPSE_SHOT，不输出 CYBERBRICK_SHOT。Klipper
侧会保留旧宏兼容，因此切片器不需要双发；双发会造成歧义。

Off：
- 0 个 ESP32_TIMELAPSE_SHOT。
- 启用 ESP32 backend 后由它完整接管延时摄影；Off 也不得落入旧
  time_lapse_gcode 或 TIMELAPSE_TAKE_FRAME 造成重复。

Traditional：
- 完成当前整层后恰好一帧，层号连续。
- 严格顺序：M400 -> ESP32_TIMELAPSE_SHOT -> G4 P<wait>。
- wait 最小 1000 ms，默认 1200 ms；GUI 校验和 G-code 入口都拒绝更小值。
- 不停车、不生成稳定塔。
...
ESP32_TIMELAPSE_SHOT。不要改它们的层首拍摄、A1 成品帧或 3MF 0/1 契约。
