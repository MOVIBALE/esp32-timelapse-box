...
这是工程预发布版。配置器压缩包同时包含 S3 Sony BLE 路线和兼容型 C3 路线；S3 合并固件从 `0x0` 地址刷入。项目自有源码使用 `GPL-3.0-only`，分发修改版时必须遵守 GPLv3 并提供对应源码。当前 135 层实机测试已经完成，但整合后的“清空蓝牙配对记录后首次配对”仍需专项复测，因此首次配对功能暂按实验能力发布。任何真实打印都应先完成 dry-run，再进入 armed。
