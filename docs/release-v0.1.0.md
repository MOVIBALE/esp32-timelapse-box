...
- Canonical `ESP_TIMELAPSE_SHOT` macro and compatible `CYBERBRICK_SHOT` alias sharing one event sequence.
...
- Safe bonded-Sony automatic reconnect on boot; the board remains unarmed and
  performs no shutter write while reconnecting.
- Bidirectional Web Serial over the board's single USB-C port.

## License

Project-owned source is licensed under `GPL-3.0-only`. Anyone distributing a modified version or product containing this program must comply with GPLv3, including providing the corresponding source. SnapOrca remains governed by its own AGPL-3.0 license.

## Verified

- A real multi-material Smooth print produced 135 expected frames from 135 layers,
  with the toolhead parked near `X250 Y240` for each external frame.
- U1 native timelapse and ESP32/Sony capture ran together without conflicting.
- Power-cycle testing restored the bonded Sony connection to `ready=true` while
  remaining `dry_run=true, armed=false` and taking no photo during reconnect.
- ESP32-S3 firmware builds with PlatformIO and ESP-IDF.
- Browser connection only reads S3 status; safety-mode changes require an
...
这是工程预发布版。配置器压缩包同时包含 S3 Sony BLE 路线和兼容型 C3 路线；S3 合并固件从 `0x0` 地址刷入。项目自有源码使用 `GPL-3.0-only`，分发修改版时必须遵守 GPLv3 并提供对应源码。当前 135 层多材料平滑模式实机测试已经完成，原生延时摄影与 ESP32/Sony 双路同时工作正常；已有配对记录的相机会在开机后安全重连，但不会自动拍照。整合后的“清空蓝牙配对记录后首次配对”仍需专项复测，因此首次配对功能暂按实验能力发布。任何真实打印都应先完成 dry-run，再进入 armed。
